...
function analyzeFile(filePath, config) {
  try {
    ...
    documentable = documentable.filter((item, index, self) => 
    ...
    const inlineDocPattern = /\/\/\s*@\w+|\/\/\s*TODO:|\/\/\s*FIXME:|\/\/\s*NOTE:/g;
    ...
    const result = {
      ...
      coverage: documentable.length > 0 ? (jsdocComments.length / documentable.length) * 100 : 100,
      breakdown: {
        ...
        reactComponents: documentable.filter(d => d.type === 'reactComponents').length
      },
      jsdocQuality,
...
function analyzeJSDocQuality(jsdocComments) {
  ...
  if (quality.total > 0) {
    quality.score = Math.round(
      ((quality.withDescription + quality.withParams + quality.withReturns + quality.withExamples) / 
       (quality.total * 4)) * 100
    );
  }
...
function generateReport(config) {
  ...
  if (files.length === 0) {
    console.error(`${colors.red}No files found matching pattern: ${pattern}${colors.reset}`);
    process.exit(1);
  }
  ...
  const overallCoverage = totalDocumentable > 0 ? (totalDocumented / totalDocumentable) * 100 : 100;
  ...
  const report = {
    ...
    breakdown: {
      ...
      reactComponents: results.reduce((sum, r) => sum + r.breakdown.reactComponents, 0)
    },
...
function outputConsole(report) {
  ...
  const coverageColor = summary.overallCoverage >= 90 ? colors.green : 
                       summary.overallCoverage >= 70 ? colors.yellow : colors.red;
  
  console.log(`\n${colors.bright}📚 Documentation Coverage Report${colors.reset}`);
  console.log(`${'='.repeat(50)}`);
  console.log(`${colors.bright}Overall Coverage:${colors.reset} ${coverageColor}${summary.overallCoverage.toFixed(1)}%${colors.reset}`);
  console.log(`${colors.bright}Files Analyzed:${colors.reset} ${summary.totalFiles}`);
  console.log(`${colors.bright}Total Documentable:${colors.reset} ${summary.totalDocumentable}`);
  console.log(`${colors.bright}Total Documented:${colors.reset} ${summary.totalDocumented}`);
  console.log(`${colors.bright}Threshold:${colors.reset} ${report.config.threshold}%`);
  console.log(`${colors.bright}Status:${colors.reset} ${summary.passesThreshold ? `${colors.green}✅ PASS` : `${colors.red}❌ FAIL`}${colors.reset}`);
  ...
  if (worstFiles.length > 0) {
    console.log(`${'File'.padEnd(40)} ${'Coverage'.padEnd(10)} ${'Items'.padEnd(8)} ${'Docs'.padEnd(6)}`);
    console.log(`${'-'.repeat(70)}`);
    
    worstFiles.forEach(file => {
      const coverageStr = `${file.coverage.toFixed(1)}%`;
      const fileColor = file.coverage >= 90 ? colors.green : 
                       file.coverage >= 70 ? colors.yellow : colors.red;
...
function outputMarkdown(report) {
  ...
  files.filter(f => f.documentable > 0).slice(0, 15).forEach(file => {
    const status = file.coverage >= 90 ? '🟢' : file.coverage >= 70 ? '🟡' : '🔴';
  ...
  console.log(`${colors.green}Markdown report saved to: ${filename}${colors.reset}`);
}
...
<body>
    <div class="container">
        ...
        <table>
            ...
            <tbody>
                ${Object.entries(breakdown).map(([type, count]) => 
                    count > 0 ? `<tr><td>${type}</td><td>${count}</td></tr>` : ''
                ).join('')}
            </tbody>
        </table>
        ...
        <table>
            ...
            <tbody>
                ${files.filter(f => f.documentable > 0).map(file => {
                    const coverageClass = file.coverage >= 90 ? 'coverage-high' : 
                                         file.coverage >= 70 ? 'coverage-medium' : 'coverage-low';
                    const progressClass = file.coverage >= 90 ? 'progress-high' : 
                                         file.coverage >= 70 ? 'progress-medium' : 'progress-low';
                    return `
                        <tr>
                            <td>${file.file}</td>
  ...
  console.log(`${colors.green}HTML report saved to: ${filename}${colors.reset}`);
}
